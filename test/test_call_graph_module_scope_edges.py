...
it built `app_symbols` from callables and classes only. A resolver attributes a call in
...
def test_module_scope_call_to_a_library_target_is_kept():
    st = _symbol_table()
    edges = [PyCallEdge(src="app", dst="functools.reduce", weight=1, prov=["defuse"])]
...
def test_module_scope_decorator_application_is_kept():
    ...
    st = _symbol_table()
    edges = [
        PyCallEdge(src="app", dst="functools.lru_cache", weight=1, prov=["defuse"]),
        PyCallEdge(src="app", dst="functools.cache", weight=1, prov=["defuse"]),
    ]
    assert filter_external_edges(edges, st) == edges
...
def test_a_genuine_library_to_library_edge_is_still_dropped():
    ...
    st = _symbol_table()
    edges = [PyCallEdge(src="os.path.join", dst="os.sep", weight=1, prov=["defuse"])]
...
def test_callable_endpoints_are_unaffected():
    ...
    edges = [
        ...
        PyCallEdge(src="app", dst="app.in_a_function", weight=1, prov=["defuse"]),
    ]
    assert filter_external_edges(edges, st) == edges
...
def test_a_module_named_like_a_library_is_still_an_app_symbol():
    ...
    edges = [PyCallEdge(src="functools", dst="os.getcwd", weight=1, prov=["defuse"])]
    assert filter_external_edges(edges, st) == edges
